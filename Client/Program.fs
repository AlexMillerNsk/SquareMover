...
let update (msg:Msg) (model:Model) =
    ...
    | OnDragEnd ev   -> let textfordrop = document.getElementById("textfordrop")
                        ...
                        let itog  = droptarget.appendChild textfordrop                        
...
let draggableSmthn (model:Model) (dispatch: Msg -> unit) =
    Html.div [
      prop.style [style.backgroundColor "red"; style.width 450]
      prop.children [
          Html.p [
          prop.id "textfordrop"
...
let dropTarget =
    Html.div [
    prop.style [style.width 450; style.height 300; style.left 360; style.top 260; style.backgroundColor "yellow"]
    prop.id "droptarget"
    prop.classes []
    prop.draggable true
    prop.text "23x" ]
